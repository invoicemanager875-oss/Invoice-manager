<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-xl font-bold text-navy-600">Buat Surat Perjanjian Kerja (SPK)</h2>
                <p class="text-sm text-slate-500 mt-0.5">Penyusunan dokumen kontrak legalitas kerja proyek resmi ber-Kop Brand</p>
            </div>
            <a href="{{ route('contracts.index') }}" class="text-sm text-slate-500 hover:text-slate-700">
                &larr; Kembali
            </a>
        </div>
    </x-slot>
    ...
    @php
        ...
        $initData = $initialData ?? [
            'brand_id' => $brands->first()->id ?? null,
            'invoice_id' => request('invoice_id', ''),
            'judul_kontrak' => 'Surat Perjanjian Kerja',
            'tanggal_kontrak' => date('Y-m-d'),
            'pihak_pertama_nama' => $brands->first()->ttd_nama ?? 'Pimpinan Brand',
            'pihak_pertama_jabatan' => $brands->first()->ttd_jabatan ?? 'Direktur',
            'pihak_pertama_perusahaan' => $brands->first()->name ?? '',
            'pihak_pertama_alamat' => $brands->first()->address ?? '',
            'pihak_pertama_telepon' => $brands->first()->phone ?? '',
            'pihak_kedua_nama' => '',
            ...
            'catatan' => '',
            'scopes' => [
                ['nama_paket' => 'Perencanaan Arsitektur & Desain Fasad', 'deskripsi' => '3D Render, Gambar Kerja, RAB', 'nominal' => 0]
            ],
            'terms' => [
                ['judul' => 'Termin I (Uang Muka / DP)', 'persentase' => 50, 'nominal' => 0, 'syarat_pencairan' => 'Saat penandatanganan kontrak'],
                ['judul' => 'Termin II (Pelunasan)', 'persentase' => 50, 'nominal' => 0, 'syarat_pencairan' => 'Setelah penyerahan seluruh berkas final']
            ]
        ];
    @endphp
    ...
    <div x-data="{
        ...
        brandId: '{{ old('brand_id', $initData['brand_id']) }}',
        p1Nama: '{{ addslashes(old('pihak_pertama_nama', $initData['pihak_pertama_nama'])) }}',
        p1Jabatan: '{{ addslashes(old('pihak_pertama_jabatan', $initData['pihak_pertama_jabatan'])) }}',
        p1Perusahaan: '{{ addslashes(old('pihak_pertama_perusahaan', $initData['pihak_pertama_perusahaan'])) }}',
        p1Alamat: '{{ addslashes(old('pihak_pertama_alamat', $initData['pihak_pertama_alamat'])) }}',
        p1Telepon: '{{ addslashes(old('pihak_pertama_telepon', $initData['pihak_pertama_telepon'])) }}',

        p2Nama: '{{ addslashes(old('pihak_kedua_nama', $initData['pihak_kedua_nama'])) }}',
        p2Alamat: '{{ addslashes(old('pihak_kedua_alamat', $initData['pihak_kedua_alamat'])) }}',
        p2Telepon: '{{ addslashes(old('pihak_kedua_telepon', $initData['pihak_kedua_telepon'])) }}',

        nilaiKontrak: {{ old('nilai_kontrak', $initData['nilai_kontrak']) }},
        durasiHari: {{ old('durasi_hari', $initData['durasi_hari']) }},
        tglMulai: '{{ old('tanggal_mulai', $initData['tanggal_mulai']) }}',
        tglSelesai: '{{ old('tanggal_selesai', $initData['tanggal_selesai']) }}',
        narasi: `{{ addslashes(old('narasi', $initData['narasi'])) }}`,

        scopes: {{ Js::from(old('scopes', $initData['scopes'])) }},
        terms: {{ Js::from(old('terms', $initData['terms'])) }},
        ...
        <form action="{{ route('contracts.store') }}" method="POST" class="space-y-6">
            ...
            {{-- Load from Invoice Card --}}
            <div class="bg-gradient-to-r from-sky-50 to-indigo-50 border border-sky-200 rounded-xl p-4 shadow-sm flex items-center justify-between flex-wrap gap-3">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-lg bg-sky-600 text-white flex items-center justify-center font-bold">
                        📄
                    </div>
                    <div>
                        <div class="font-bold text-sm text-navy-800">Muat Otomatis Data dari Invoice</div>
                        <div class="text-xs text-slate-500">Pilih invoice yang sudah ada untuk mengisi otomatis data klien, total biaya, dan termin</div>
                    </div>
                </div>

                <div class="flex items-center gap-2">
                    <select onchange="if(this.value) window.location.href='{{ route('contracts.create') }}?invoice_id=' + this.value"
                            class="text-xs border-slate-300 rounded-lg shadow-sm focus:border-sky-500 focus:ring-sky-500">
                        <option value="">-- Pilih Invoice Referensi --</option>
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="judul_kontrak" value="{{ old('judul_kontrak', $initData['judul_kontrak']) }}" required
                               class="w-full text-sm border-slate-300 focus:border-navy-500 focus:ring-navy-500 rounded-lg shadow-sm">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_kontrak" value="{{ old('tanggal_kontrak', $initData['tanggal_kontrak']) }}" required
                               class="w-full text-sm border-slate-300 focus:border-navy-500 focus:ring-navy-500 rounded-lg shadow-sm">
                    </div>
                </div>
            </div>
                    ...
                    <div class="space-y-3 text-xs">
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <div>
                                ...
                                <input type="text" name="pihak_kedua_identitas" value="{{ old('pihak_kedua_identitas', $initData['pihak_kedua_identitas']) }}"
                                       class="w-full text-sm border-slate-300 rounded-lg shadow-sm">
                            </div>
                        </div>
                        <div>
                            <label class="block font-semibold text-slate-600 mb-1">Nama Perusahaan (Opsional)</label>
                            <input type="text" name="pihak_kedua_perusahaan" value="{{ old('pihak_kedua_perusahaan', $initData['pihak_kedua_perusahaan']) }}"
                                   class="w-full text-sm border-slate-300 rounded-lg shadow-sm">
                        </div>
                        <div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <div>
                                ...
                                <input type="email" name="pihak_kedua_email" value="{{ old('pihak_kedua_email', $initData['pihak_kedua_email']) }}"
                                       class="w-full text-sm border-slate-300 rounded-lg shadow-sm">
                            </div>
                        </div>
                    </div>
            ...
            <div class="flex items-center justify-end gap-3">
                <a href="{{ route('contracts.index') }}" class="px-5 py-2.5 text-sm font-semibold text-slate-600 hover:text-slate-800">
                    Batal
                </a>
