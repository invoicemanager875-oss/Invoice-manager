<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-xl font-bold text-navy-600">Edit Jadwal Perencanaan</h2>
                <p class="text-sm text-slate-500 mt-0.5">{{ $jadwal->nama_proyek }} &middot; {{ $jadwal->brand->name ?? '-' }}</p>
            </div>
            <a href="{{ route('jadwal-perencanaan.show', $jadwal) }}" class="text-sm text-slate-500 hover:text-slate-700">
    ...
    <div x-data="{
        durasi: {{ $jadwal->durasi_hari }},
        items: {{ Js::from($jadwal->items->map(fn($item) => [
            'nama_item' => $item->nama_item,
            'bobot' => (float) $item->bobot,
            'hari_mulai' => (int) $item->hari_mulai,
            'hari_selesai' => (int) $item->hari_selesai,
        ])) }},
        addItem() {
            this.items.push({
                nama_item: '',
        ...
        <form action="{{ route('jadwal-perencanaan.update', $jadwal) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="brand_id" required class="w-full text-sm border-slate-300 focus:border-navy-500 focus:ring-navy-500 rounded-lg shadow-sm">
                            ...
                            @foreach ($brands as $brand)
                                <option value="{{ $brand->id }}" @selected(old('brand_id', $jadwal->brand_id) == $brand->id)>{{ $brand->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_proyek" value="{{ old('nama_proyek', $jadwal->nama_proyek) }}" required
                    ...
                    <div>
                        ...
                        <select name="invoice_id" class="w-full text-sm border-slate-300 focus:border-navy-500 focus:ring-navy-500 rounded-lg shadow-sm">
                            ...
                            @foreach ($invoices as $inv)
                                <option value="{{ $inv->id }}" @selected(old('invoice_id', $jadwal->invoice_id) == $inv->id)>
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_mulai" value="{{ old('tanggal_mulai', $jadwal->tanggal_mulai ? $jadwal->tanggal_mulai->format('Y-m-d') : '') }}" required
                               class="w-full text-sm border-slate-300 focus:border-navy-500 focus:ring-navy-500 rounded-lg shadow-sm">
                ...
                <div>
                    ...
                    <textarea name="catatan" rows="2"
                              class="w-full text-sm border-slate-300 focus:border-navy-500 focus:ring-navy-500 rounded-lg shadow-sm">{{ old('catatan', $jadwal->catatan) }}</textarea>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-3">
                <a href="{{ route('jadwal-perencanaan.show', $jadwal) }}" class="px-5 py-2.5 text-sm font-semibold text-slate-600 hover:text-slate-800">
                    Batal
                </a>
